...
def load_captions_from_json(captions_path: str) -> tuple[List[str], List[str], List[int]]:
    """Load ALL captions from a JSON file (multiple per sample).

    Args:
        captions_path: Path to captions JSON file

    Returns:
        Tuple of (sample_ids, captions, caption_indices)
        - sample_ids: repeated for each caption (e.g., ['id1', 'id1', 'id1', 'id1', 'id2', ...])
        - captions: all captions flattened (e.g., [cap1_0, cap1_1, cap1_2, cap1_3, cap2_0, ...])
        - caption_indices: which caption # for each (e.g., [0, 1, 2, 3, 0, 1, 2, 3, ...])
    """
    ...
    for sample in samples:
        ...
        # Add each caption with its index
    ...
    return sample_ids, captions, caption_indices
...
def encode_captions(
    ...
    # Load captions (ALL captions per sample, not just first one)
    print(f"\n📖 Loading captions from: {captions_path}")
    sample_ids, captions, caption_indices = load_captions_from_json(captions_path)

    # Calculate statistics
    unique_samples = len(set(sample_ids))
    avg_captions_per_sample = len(captions) / unique_samples if unique_samples > 0 else 0
    print(f"   Found {len(captions)} captions from {unique_samples} samples")
    print(f"   Average captions per sample: {avg_captions_per_sample:.1f}")

    # Initialize encoder
    print(f"\n🔧 Initializing {config.encoder_type} encoder...")
    ...
    print(f"   ✓ Encoded {embeddings.shape[0]} captions")
    print(f"   Embedding shape: {embeddings.shape}")

    # Determine output path
    if output_path is None:
        output_path = encoder.get_output_path(captions_path, output_dir)
        print(f"\n📍 Auto-generated output path: {output_path}")
    ...
    # Calculate captions per sample for metadata
    unique_samples = len(set(sample_ids))
    captions_per_sample = len(captions) / unique_samples if unique_samples > 0 else 1

    # Save embeddings with metadata (includes caption indices for multi-caption support)
    np.savez_compressed(
        output_path,
        embeddings=embeddings,
        sample_ids=np.array(sample_ids),
        caption_indices=np.array(caption_indices),  # NEW: track which caption # (0, 1, 2, 3...)
        # Metadata
        ...
        num_unique_samples=np.array([unique_samples]),
        avg_captions_per_sample=np.array([captions_per_sample])
    )

    print(f"   ✓ Saved embeddings ({embeddings.nbytes / 1024 / 1024:.2f} MB)")

    print("\n" + "=" * 80)
    print("✨ Caption encoding completed successfully!")
    print("=" * 80 + "\n")
