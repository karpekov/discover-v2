"""DistilRoBERTa text encoder.

Uses distilroberta-base model from HuggingFace.
"""
...
class DistilRoBERTaTextEncoder(BaseTextEncoder):
    """Frozen DistilRoBERTa text encoder.
    
    Uses distilroberta-base (768-d embeddings) by default.
    Embeddings are extracted via mean pooling and L2-normalized.
    """
    
    def __init__(self, config: TextEncoderConfig):
        """Initialize DistilRoBERTa encoder.
    ...
    def _mean_pooling(self, token_embeddings: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        # Sum embeddings and divide by the number of tokens
