...
Natural language captions generated by Large Language Models with pluggable backend support.

**Status:** ✅ **IMPLEMENTED** - Full support for OpenAI, Gemini, Gemma, and Llama.

**Features:**
- Multiple backends: OpenAI (GPT), Google (Gemini), Local (Gemma, Llama)
- Compact JSON representation optimized for LLM input
- Unified prompting system across all backends
...
**Quick Start:**
```bash
# Debug mode - test with 3 samples
...
**Config:** `configs/captions/llm_openai.yaml`, `llm_gemini.yaml`, `llm_gemma.yaml`, `llm_llama.yaml`

**Documentation:** See `docs/LLM_CAPTION_GENERATION.md` for full details.
