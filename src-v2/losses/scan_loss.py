"""
SCAN Loss implementation for clustering training.
Adapted from: https://github.com/wvangansbeke/Unsupervised-Classification/
"""
import torch
...
class SCANLoss(nn.Module):
    ...
    def forward(self, anchors_logits, neighbors_logits):
        """
        input:
            - anchors_logits: logits for anchor samples w/ shape [b, num_classes]
            - neighbors_logits: logits for neighbor samples w/ shape [b, num_classes]
        
        output:
            - Loss components
        """
        # Softmax
        b, n = anchors_logits.size()
        anchors_prob = self.softmax(anchors_logits)
        positives_prob = self.softmax(neighbors_logits)
...
class ConfidenceBasedCE(nn.Module):
    ...
    def forward(self, anchors_weak, anchors_strong):
        ...
        if self.apply_class_balancing:
            ...
            weight = torch.ones(c).to(input_.device)
            weight[idx] = freq
